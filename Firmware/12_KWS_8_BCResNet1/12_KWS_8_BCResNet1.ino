...
void setup() {
  ...
  model = tflite::GetModel(bcresnet3);
  ...
  // NOLINTNEXTLINE(runtime-global-variables)
  static tflite::MicroMutableOpResolver<20> micro_op_resolver(error_reporter);
...
void loop() {
  ...
  //                 is_new_command);
  uint8_t y_val = argmax(output->data.int8, kCategoryCount);
  Serial.println((String)y_val + " score: " + (String)(output->data.int8[y_val]) + " label: " + kCategoryLabels[y_val]);
  //delay(1000);
}

uint8_t argmax(int8_t *input_scores, uint8_t num_classes)
{
    int8_t max_value = 0;
    uint8_t idx;
